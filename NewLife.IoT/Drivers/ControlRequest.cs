...
/// <summary>控制请求。用于 ControlAsync 发起结构化控制调用</summary>
/// <remarks>
/// 替代裸字典参数，提供服务名、参数和扩展元数据三层结构，让控制调用语义更清晰。
/// <para>与 <see cref="NewLife.IoT.Models.ServiceRequest"/> 的区别：Models 版用于平台层下发（含 DeviceId、Timeout 等调度字段），
/// 本类用于驱动层执行时的参数传递。</para>
/// <example>
/// <code>
/// var req = ControlRequest.Create("Calibrate", new Dictionary&lt;String, Object?&gt;
/// {
///     ["Zero"] = 0.0,
///     ["Span"] = 100.0
/// });
...
public class ControlRequest : IExtend
{
    ...
    public static ControlRequest Create(String serviceName, IDictionary<String, Object?>? parameters = null)
        => new() { ServiceName = serviceName, Parameters = parameters ?? new Dictionary<String, Object?>() };
    #endregion

    /// <summary>以文本方式输出请求摘要</summary>
    public override String ToString() => $"ControlRequest({ServiceName}, {Parameters.Count} params)";
}
